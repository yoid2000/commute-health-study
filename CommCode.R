...
#    airDistLog2=aggregate(dd[, "airDistLog2"], list(dd$commGrp, dd$gender), median)$x)
...
summary(m)
new= data.frame(gender=c(rep(1,4), rep(2,4)), CommHome=0:3, age=mean(dd$age), MVPAsqrt=mean(dd$MVPAsqrt),
                DistLog2Home=aggregate(dd[, "DistLog2Home"], list(dd$CommHome), median)$x)
new$gender= factor(new$gender);  levels(new$gender)=levels(dd$gender)
