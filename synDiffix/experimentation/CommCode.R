...
csv_files <- list.files(path = csv_dir, pattern = "*.csv", full.names = FALSE)
print(csv_files)
print("------------------------")

# Function to extract the suffix from the file name
extract_suffix <- function(filename) {
  str_match(filename, "(.*)_2(home|school)\\.csv")[, 2]
}

# Find pairs of files with the same suffix
suffixes <- unique(sapply(csv_files, extract_suffix))
print(suffixes)
print("--------------------------")
for (suffix in suffixes) {
  home_file <- file.path(csv_dir, paste0(suffix, "_2home.csv"))
  school_file <- file.path(csv_dir, paste0(suffix, "_2school.csv"))
  
  if (file.exists(home_file) && file.exists(school_file)) {
    # Define the output file names
    all_json <- file.path(result_dir, paste0(suffix, ".json"))
    plot_png <- file.path(result_dir, paste0(suffix, "_plot.png"))
    
    # Call the runComputation function
    runComputation(school_file, home_file, all_json, plot_png)
    print(school_file)
    print(home_file)
    print(all_json)
    print(plot_png)
